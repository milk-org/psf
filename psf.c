...
errno_t PSF_finddiskcent_alone(
    const char *ID_name,
    float       rad
)
{
...
errno_t PSF_measurePhotocenter(
    const char *ID_name
)
{
    imageID  ID;
...
)
{
    ...
    for(i = 0; i < nb_step; i++)
    {
        ...
        {
            FWHM = 2.0 * dist[i] + (dist[i + 1] - dist[i]) * (mean[i] - mean[0] / 2) /
                   (mean[i] - mean[i + 1]);
        }
    }
...
)
{
    ...
    {
        n3 = (long)(1.0 * naxes[0] / 2 / (1.0 + (0.1 * naxes[0] / 2 * k /
                                          (4 * nbiter))));
        ...
        for(uint32_t jj = jjstart; jj < jjend; jj++)
            for(uint32_t ii = iistart; ii < iiend; ii++)
            {
                ...
                {
                    centerx += 1.0 * ii * (data.image[ID].array.F[jj * naxes[0] + ii] - 1.0 *
                                           back_cont);
                    centery += 1.0 * jj * (data.image[ID].array.F[jj * naxes[0] + ii] - 1.0 *
                                           back_cont);
...
)
{
    ...
    while((i < max_nb_iter) && (found == 0))
    {
        ...
        for(uint32_t jj = (n2 - n3); jj < (n2 + n3); jj++)
            ...
            {
                coeff = (ii - x_star[0]) * (ii - x_star[0]) + (jj - y_star[0]) *
                        (jj - y_star[0]);
                coeff = coeff / n3 / n3;
        ...
        xsum  = xsum / sum;
        ysum  = ysum / sum;
        if(((x_star[0] - xsum) * (x_star[0] - xsum) + (y_star[0] - ysum) *
                (y_star[0] - ysum)) < (limit * limit * n3 * n3))
        {
            found = 1;
        }
        x_star[0] = xsum;
...
)
{
    ...
    for(int i = 0; i < 100; i++)
        if(count[i] > 0)
        {
            err = err + pow((value[i] - C - A * exp(-dist[i] * dist[i] / sigmasq)),
                            2) * count[i];
        }
    best_err = err;
    ...
    peak = value[0] - C;
    {
        ...
        FWHM_m = 2.0 * dist[i - 1] + 2.0 * (dist[i] - dist[i - 1]) *
                 (value[i - 1] - C - peak / 2.0) / (value[i - 1] - value[i]);
    }
...
)
{
    ...
    arith_image_extract2D(ID_name, "tmpf", size, size,
...
)
{
    ...
    for(ii1 = 0; ii1 < size; ii1++)
        for(jj1 = 0; jj1 < size; jj1++)
        {
            ...
            {
                data.image[IDout].array.F[jj1 * size + ii1] = data.image[IDin].array.F[jj *
                        naxes[0] + ii];
            }
            else
            {
...
)
{
    ...
    for(uint32_t jj = 0; jj < naxes[1]; jj++)
        for(uint32_t ii = 0; ii < naxes[0]; ii++)
        {
            dist = sqrt((ii - naxes[0] / 2) * (ii - naxes[0] / 2) + (jj - naxes[1] / 2) *
                        (jj - naxes[1] / 2));
            data.image[IDout].array.F[jj * naxes[0] + ii] =
                exp(-pow(dist / radius, index)) * data.image[ID1].array.F[jj * naxes[0] + ii]
                + (1.0 - exp(-pow(dist / radius,
                                  index))) * data.image[ID2].array.F[jj * naxes[0] + ii];
        }
...
)
{
    ...
    {
        arith_image_extract2D(ID_name, "tmpsr", Csize, Csize,
                              ((long) xcenter[0]) - Csize / 2, ((long) ycenter[0]) - Csize / 2);
        ID = image_ID("tmpsr");
        peak = 0.0;
...
)
{
    ...
    for(kk = 0; kk < ksize; kk++)
    {
        ...
        {
            flux_array[kk] -= data.image[IDcin].array.F[kk * xsize * ysize + ii] *
