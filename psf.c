...
static errno_t init_module_CLI()
{
    RegisterCLIcommand("psfseqmeas", __FILE__, PSF_sequence_measure_cli, "measure PSF sequence",
                       "<input image cube> <estimated PSF size> <output file>", "psfseqmeas imc 20.0 outimc.txt",
...
imageID PSF_makeChromatPSF(const char *amp_name, const char *pha_name, float coeff1, float coeff2, long NBstep,
                           float ApoCoeff, const char *out_name)
{
    imageID IDin;
    imageID IDout;
...
errno_t PSF_finddiskcent(const char *ID_name, float rad, float *result)
{
    ...
    {
        fprintf(stderr, "iter %ld / %ld  (%f %f  %f %f   %f %f) %g\n", iter, NBiter, xcstart, xcend, ycstart, ycend,
                xcb, ycb, bvalue);
...
errno_t center_PSF_alone(const char *ID_name)
{
    ...
    arith_image_trunc("tmpcen", arith_image_percentile("tmpcen", 0.99), arith_image_percentile("tmpcen", 1.0),
                      "tmpcen1");
...
float get_sigma(const char *ID_name, float x, float y, const char *options)
{
    ...
    peak = value[0] - C;
    {
        ...
        FWHM_m = 2.0 * dist[i - 1] +
                 2.0 * (dist[i] - dist[i - 1]) * (value[i - 1] - C - peak / 2.0) / (value[i - 1] - value[i]);
    }
...
float get_sigma_alone(const char *ID_name)
{
    ...
    if (FAST == 0)
    {
        arith_image_trunc("tmpcen", arith_image_percentile("tmpcen", 0.9), arith_image_percentile("tmpcen", 1.0),
                          "tmpcen1");
        delete_image_ID("tmpcen", DELETE_IMAGE_ERRMODE_WARNING);
        center_PSF("tmpcen1", xcenter, ycenter, box_size);
        delete_image_ID("tmpcen1", DELETE_IMAGE_ERRMODE_WARNING);
    ...
    for (uint32_t jj = 0; jj < naxes[1]; jj++)
        for (uint32_t ii = 0; ii < naxes[0]; ii++)
        {
            ...
            data.image[IDout].array.F[jj * naxes[0] + ii] =
                ...
                (1.0 - exp(-pow(dist / radius, index))) * data.image[ID2].array.F[jj * naxes[0] + ii];
        }
...
float psf_measure_SR(const char *ID_name, float factor, float r1, float r2)
{
    ...
    if ((xcenter[0] < Csize / 2 + 1) || (xcenter[0] > naxes[0] - Csize / 2 - 1) || (ycenter[0] < Csize / 2 + 1) ||
        (ycenter[0] > naxes[1] - Csize / 2 - 1))
    {
